...
namespace Dui
{
    ...
    void push_vector_text(DrawList *dl, VectorFont *font, i32 z, String text, Vec2f pos, Color color,
                          f32 size)
    {
        for (int i = 0; i < text.size; i++) {
    ...
    void init_draw_system(DrawList *dl, Device *device, Pipeline pipeline)
    {
        ...
        for (i32 i = 0; i < dl->icon_font.curves.size; i++) {
            dl->primitives.conic_curves[dl->conic_curves_count++] = {
                dl->icon_font.curves[i].p0, dl->icon_font.curves[i].p1, dl->icon_font.curves[i].p2};
        }
